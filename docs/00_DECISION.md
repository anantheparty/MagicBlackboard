...
- 状态：**Accepted**
- 决策日期：2026-08-12
- 适用里程碑：Foundation（历史基线）与 Pressure Ink V2（当前模拟实现阶段）
...
Foundation 已在 merge commit `21cc3a9abbd5f28999c7e44fe03c8b69fe9ae0d4` 完成并保留证据。
Pressure Ink V2 的模拟/browser 代码已在当前 working branch 实现，只在默认关闭、向后兼容
和数据最小化边界内提供压感笔迹；最终固定 commit 门禁/hosted CI 仍待记录，且尚无真实
Safari/iPad/Apple Pencil/数位板证据。AI 与自动 Actor 仍不属于当前里程碑。
...
| D-012 | Pressure Ink V2 采用 exact optional `{ version: 1, widths }` 最终宽度几何；保留旧 `points`/`strokeWidth`，两个 dev feature 分离且默认 off，production unavailable，平台结论保持 Unverified | 让旧 Drawnix/default/fallback 路径保持可用，只保存重绘所需信息，不把原始 pressure/event/device 标识写入文档 | 详见 [`adr/0001-pressure-ink-v2-data-and-boundaries.md`](./adr/0001-pressure-ink-v2-data-and-boundaries.md)；任何破坏旧格式、production rollout、默认开启或原生 renderer 的方案都需要新 ADR |
...
这些条目描述的是 Foundation 的历史边界。Pressure Ink V2 只解除了其中的 pressure-ink
模拟/browser 实现禁令；模型、Actor、协作、云同步、production rollout、原生 iPadOS 与第二
renderer 仍是明确非目标。Safari/iPad、物理 Apple Pencil/数位板和真实 PWA 验证均未因代码
实现而自动完成。
